...
const SKILLS={
    ...
    'Infinity':          {cost:120, type:'instant', color:'#00d2ff', slot:'Ability', desc:'ABILITY — Activate Infinity to block ALL enemy attacks (chess captures and all attacking skills) until your next turn starts. Bypassed by: Mahoraga (after adapting), Heavenly Restriction, attacker has an active domain, or Domain Clash.'},
    ...
    'Rika':              {cost:150, type:'instant', color:'#fd79a8', slot:'Ability', desc:'ABILITY — Summon Rika as a bonus Queen piece on your back rank. While Rika is on the board, ALL your skill costs are halved (stacks with Six Eyes for 25% total). Rika acts as a normal queen in chess. Press again to retract for free. If Rika is captured she cannot return.'},
    ...
    'Idle Transfiguration':{cost:150,type:'target-own',color:'#b040ff',slot:'Ability',desc:'ABILITY — Mahito\'s soul manipulation. Click one of your own pieces or pawns (not King, not Queen) to instantly transform it into a Queen. This does not consume your turn — you still get to make a chess move after.'},
...
function renderCharDrops(){
    const allDrops={
        ...
        'Gojo Satoru (Strongest)':['Lapse Blue','Six Eyes','Hollow Purple','Limitless','Infinite Void','Reverse Cursed Technique'],
...
function getOppSkillPool(opp){
    const pools={
        ...
        'Gojo Satoru (Strongest)':['Hollow Purple','Reversal Red','Lapse Blue','Infinite Void','Reverse Cursed Technique'],
        'Okkotsu Yuta':['Cursed Speech','Copy','Reverse Cursed Technique','Rika','True Mutual Love'],
    };
    return pools[opp]||[];
}
function executeTech(name,isAI,r,c){
    ...
    if(name==='Infinity'){
        ...
        // Infinity (player): AI Dismantle blocked — Mahoraga pierces when adapted
        if(isAI&&state.infP>0&&!isDomainClash()){
            if(state.mahoragaAdaptedLimitless){
                showMahoragaWheel();log('Mahoraga pierces through Infinity!');
            } else {
        ...
        // Limitless (player passive): AI Dismantle blocked — Mahoraga pierces when adapted
        if(isAI&&!isDomainClash()){
            const hasLimitless=Object.values(prog.eq).some(v=>v==='Limitless')&&!isDivinaSealed('Limitless');
            if(hasLimitless&&!state.mahoragaAdaptedLimitless){
        ...
        if(!isAI&&state.opp==='Gojo Satoru (Strongest)'&&!state.mahoragaAdaptedLimitless&&!isDomainClash()){
        ...
        // Infinity / Limitless blocks direct attacks (but not domains)
        if(isAI&&!isDomainClash()){
            const rTarget=state.board[r]?.[c];
            if(rTarget?.color==='W'){
                // Infinity check
                if(state.infP>0){
                    if(state.mahoragaAdaptedLimitless){showMahoragaWheel();log('Mahoraga pierces through Infinity!');}
                    else{log('Infinity: Reversal Red blocked! (lasts until end of turn)');if(isAI) state.ceE+=getTechCost(name,true);render();if(isAI&&!state._aiNoEndTurn) endTurn();return;}
                }
                const hasLimitless=Object.values(prog.eq).some(v=>v==='Limitless')&&!isDivinaSealed('Limitless');
                if(hasLimitless){
                    const lActive=state.limitlessImmunityP>0;
        ...
        // Infinity (player) blocks AI Straw Doll
        if(isAI&&state.infP>0&&!isDomainClash()&&!state.mahoragaAdaptedLimitless){
        ...
        // Limitless (player) blocks AI Straw Doll
        if(isAI&&!isDomainClash()&&!state.mahoragaAdaptedLimitless){
            const hasLimitless=Object.values(prog.eq).some(v=>v==='Limitless')&&!isDivinaSealed('Limitless');
            if(hasLimitless){
                const hasSE=Object.values(prog.eq).some(v=>v==='Six Eyes')&&!isDivinaSealed('Six Eyes');
                const lCost=hasSE?25:50;
        ...
        // Infinity (AI) blocks player Straw Doll
        if(!isAI&&state.infE>0&&!isDomainClash()&&!state.mahoragaAdaptedLimitless){
        ...
        if(state.board[r][c]?.type!=='P'){
            ...
            if(sdRusher){
                ...
                if(state.board[r][c]?.type==='P'){
                    if(isAI&&r===7) state.board[r][c].type='Q';   // B pawn reaches player back rank
                    else if(!isAI&&r===0) state.board[r][c].type='Q'; // W pawn reaches AI back rank
                }
            }
        }
        ...
        // Infinity (player) blocks AI Divergent Fist
        if(isAI&&state.infP>0&&!isDomainClash()&&!state.mahoragaAdaptedLimitless){
            log('Infinity: Divergent Fist blocked!');state.infP=0;
            state.ceE+=getTechCost(name,true);state.casting=null;if(!state._aiNoEndTurn) endTurn();return;
        }
        // Limitless (player) blocks AI Divergent Fist
        if(isAI&&!isDomainClash()&&!state.mahoragaAdaptedLimitless){
            const hasLimitless=Object.values(prog.eq).some(v=>v==='Limitless')&&!isDivinaSealed('Limitless');
            if(hasLimitless){
                const hasSE=Object.values(prog.eq).some(v=>v==='Six Eyes')&&!isDivinaSealed('Six Eyes');
        ...
        if(!isAI&&target?.color==='B'&&target?.type==='P'){
            if(!_playerHasDomain&&state.infE>0&&!isDomainClash()&&!state.mahoragaAdaptedLimitless){
                log('Infinity: Divergent Fist blocked!');state.infE=0;
                state.ceP+=getTechCost(name);state.casting=null;render();return;
            }
            if(!_playerHasDomain&&!isDomainClash()&&!state.mahoragaAdaptedLimitless&&state.opp==='Gojo Satoru (Strongest)'){
                const lActive=state.limitlessImmunityE>0;
                const lCost=25;
                if(lActive||(state.ceE>=lCost)){
                    if(!lActive){state.ceE-=lCost;state.limitlessImmunityE=1;}
        ...
        if(target&&target.color!==color&&target.type!=='K'){
            ...
            if(!isAI&&!_pDom&&target.color==='B'&&state.opp==='Gojo Satoru (Strongest)'&&!state.mahoragaAdaptedLimitless){
                const lActive=state.limitlessImmunityE>0;
            ...
            if(isAI&&!_aDom&&target.color==='W'&&state.infP>0&&!isDomainClash()){
                if(state.mahoragaAdaptedLimitless){
                    showMahoragaWheel();log('Mahoraga pierces through Infinity!');
                } else {
                    log('Infinity: Cleave blocked! (lasts until end of turn)');
                    if(isAI) state.ceE+=getTechCost(name,true); else state.ceP+=getTechCost(name);
                    render();if(isAI&&!state._aiNoEndTurn) endTurn();return;
                }
            }
            // Player Limitless check: AI using Cleave against limitless player (bypassed if AI has domain)
            if(isAI&&!_aDom&&target.color==='W'){
                const hasLimitless=Object.values(prog.eq).some(v=>v==='Limitless')&&!isDivinaSealed('Limitless');
                if(hasLimitless&&!state.mahoragaAdaptedLimitless){
                    const lActive=state.limitlessImmunityP>0;
                    const hasSE=Object.values(prog.eq).some(v=>v==='Six Eyes')&&!isDivinaSealed('Six Eyes');
        ...
        if(!isAI){
            ...
            const _hpPDom=!!(state.infiniteVoidActive||state.playerTMLActive||state.playerSEPActive||state.playerHeianDomainActive||state.playerCSGActive||state.playerTCMPActive);
            if(!_hpPDom&&state.infE>0&&!isDomainClash()&&!state.mahoragaAdaptedLimitless){
            ...
            const _hpADom=!!(state.domain||state.gojoVoidActive||state.mahitoDomainActive||state.naoyaTCMPActive||state.csgActive||state.heianDomainActive);
            if(!_hpADom&&state.infP>0&&!isDomainClash()&&!state.mahoragaAdaptedLimitless){
                log('Infinity: Hollow Purple blocked!');state.infP=0;
                state.ceE+=getTechCost(name,true);if(!state._aiNoEndTurn) endTurn();return;
            }
            // Limitless (player) blocks AI Hollow Purple (unless AI has domain)
            if(!_hpADom&&!isDomainClash()&&!state.mahoragaAdaptedLimitless){
                const hasLimitless=Object.values(prog.eq).some(v=>v==='Limitless')&&!isDivinaSealed('Limitless');
                if(hasLimitless){
                    const hasSE=Object.values(prog.eq).some(v=>v==='Six Eyes')&&!isDivinaSealed('Six Eyes');
                    const lCost=hasSE?25:50;
    ...
    if(name==='Heian Cleave'){
        ...
        if(isAI&&!isDomainClash()&&!_hcAiDom){
            if(state.infP>0){
        ...
        const _hdAiDom=!!(state.domain||state.gojoVoidActive||state.mahitoDomainActive||state.naoyaTCMPActive||state.csgActive||state.heianDomainActive);
        if(isAI&&!isDomainClash()&&!_hdAiDom){
            if(state.infP>0){
...
function canEnemyExpandDomain(){
    ...
    if(state.opp==='Ryomen Sukuna Heian'&&state.ceE>=getTechCost('Malevolent Shrine: Heian',true)&&!state.heianDomainActive&&!state.domain&&!state.domain2) return true;
    return false;
}
        ...
        if(p){
            ...
            if(p.isAdaptive){
                ...
                if(p.isDivineDog){
                    ...
                    pieceSpan.innerText=SYMBOLS[p.color][p.type]; // standard knight symbol, silver glow via CSS
                } else if(p.isNue){
                ...
                if(p.isShadow) pieceSpan.style.opacity='0.55'; // CSG shadow clone — slightly transparent
